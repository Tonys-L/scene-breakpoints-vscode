...
```text
策略与交互层 (src/commands/*, src/codeLensProvider.ts)
    ↓
核心状态与领域层 (src/sceneStateManager.ts, src/configManager.ts, src/healingAdapter.ts, src/types.ts)
    ↑
...
- 当前状态：**已实施**
- 未隔离的模块：无（已全部通过 SRP 重构，适配器层与文件 I/O 彻底解耦，UI 状态已下沉至专用状态机）
...
| 层 | 模块 | 职责定位 | 依赖方向 | 禁止出现 |
|---|---|---|---|---|
| **策略层** | `src/commands/*`<br>`src/codeLensProvider.ts` | 处理用户交互输入（QuickPick/InputBox），调度状态机、配置层与适配层 | 依赖核心层与适配层 | 包含底层文件去重遍历细节、直接操作 VS Code DAP 接口 |
| **核心状态层** | `src/sceneStateManager.ts` | 维护全局激活场景与防竞态锁（SSOT），事件广播驱动 UI | 仅依赖 VS Code 基础 EventEmitter | 依赖任何具体 UI 控件或文件系统 I/O |
| **核心算法层** | `src/healingAdapter.ts` | 伴随特征指纹提取、双向滑动窗口加权打分算法 | 纯算法输入输出（可无缝移植） | 依赖任何外部文件 I/O 或命令交互 |
| **数据管理层** | `src/configManager.ts` | JSON 配置文件路径推导、防御性数据清洗、工作区统一推导、领域去重合并 (`upsertBreakpointToScene`) | 依赖 Node 文件系统与类型契约 | 依赖 UI 状态或断点装配下发逻辑 |
...
| 日期 | 变更内容 | 变更人 | 关联变更 |
|------|----------|--------|----------|
| 2026-09-08 | 初始版本（根据 v0.4.0 全量特性建立约束） | Tony.L | KDD-INIT-001 |
...
| 2026-09-08 | 升级全局状态机至多场景集合并落地动态多选叠加激活 (v0.5.1) | Tony.L | KDD-MULTI-ACTIVATE-001 |
| 2026-09-08 | 确立启动项联动三级匹配与幂等拦截守卫 (INV-007, v0.5.2) | Tony.L | KDD-LAUNCH-HOOK-001 |
| 2026-09-08 | 落地断点全双工同步与死循环防回环守卫 (INV-008, v0.5.3) | Tony.L | KDD-SYNC-001 |
| 2026-09-08 | 落地原生 1:1 矢量 SVG 断点与微型复选框体系，彻底根治焦点变灰 (v0.5.6) | Tony.L | KDD-UI-002 |
| 2026-09-08 | 优化标准 16x16 居中饱满矢量 SVG 图标 + 原生复选框，消除容器压缩导致的尺寸偏小 (v0.5.7) | Tony.L | KDD-UI-003 |
| 2026-09-08 | 修复无效/不存在场景名导致的状态栏虚假绿色激活，在推导层与激活层加入严格存在性校验守卫 (v0.5.8) | Tony.L | KDD-DEFENSE-001 |
| 2026-09-08 | 树节点注入稳定唯一 id 契约并拦截内部写盘冗余重刷，彻底根治启用禁用断点时的树视图重绘闪烁 (v0.5.9) | Tony.L | KDD-TREE-002 |
| 2026-09-08 | 实施领域分层 Facade 重构、SyncCoordinator 状态中枢聚合与场景批量控制/克隆增强 | Tony.L | KDD-ARCH-002 |
