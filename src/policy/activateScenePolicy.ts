...
export interface ActivateScenePolicyParams {
...
export interface ActivateScenePolicyResult {
	success: boolean;
...
/**
 * 场景激活核心用例 (Activate Scene Policy)
 * 职责：纯策略编排，杜绝任何 VS Code 弹窗或 UI 交互耦合
 * 流程：校验存在性 -> 合并断点 -> 落盘 activeScenes (SSOT) -> 装配 DAP -> 刷新状态机 -> 闭环持久化自愈
 */
export async function activateScenePolicy(
	params: ActivateScenePolicyParams,
): Promise<ActivateScenePolicyResult> {
	const {
		workspaceRoot,
	...
	// 3. SSOT 强制回写约束：先落盘 activeScenes，确保磁盘始终是权威 SSOT (INV-010)
