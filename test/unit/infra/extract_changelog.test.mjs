...
export function runExtractChangelogTests() {
	...
	const section = extractChangelogSection(enChangelogPath, "1.0.4");
	assert.ok(section.length > 0, "应能成功提取 1.0.4 章节");
	assert.ok(section.includes("Comprehensive Breakpoint Reordering"), "应包含 1.0.4 的新增内容");
	assert.ok(!section.includes("## [1.0.3]"), "不应越界包含 1.0.3 的内容");

	// 2. 带 v 前缀版本号容错
	const sectionWithV = extractChangelogSection(enChangelogPath, "v1.0.4");
	assert.equal(sectionWithV, section, "v1.0.4 与 1.0.4 提取结果应完全一致");
	...
	try {
		...
		assert.ok(notes.includes("## What's Changed in v1.0.4"), "英文标题应存在");
